        ...
        <div className="mx-auto w-full max-w-7xl px-4 md:px-6 lg:px-8">
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {services.map((s, idx) => (
              <article
                key={`${s.title}-${idx}`}
                className={`group relative flex h-full flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${s.highlight ? "border-2 border-[#ff3e00]" : "border border-slate-200"
                  }`}
              >
