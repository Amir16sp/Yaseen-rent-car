...
const DynamicCard = ({
  ...
  return (
    <>
      {/* Product Card */}
      <motion.div
        className={`rounded-lg p-1 drop-shadow-xl hover:drop-shadow-2xl transition flex flex-col justify-between bg-[#5ac9e4] backdrop-white-md ${className}`}
      ...
      >
        {/* Image */}
        <div
          className="relative h-54 sm:h-62 md:h-80 w-full mb-2"
        ...
        >
          ...
          {badgeText && (
            <span
             className="absolute top-2 right-1 bg-red-600 text-white text-sm uppercase font-semibold px-1 py-1 rounded-full z-20"
            >
              {badgeText}
           </span>
          )}
          ...
          <div className="absolute w-full bottom-0 z-10 bg-white">
            <button
              onClick={openModal}
              className="w-full bg-white text-black sm:text-sm px-5 py-3 rounded-md shadow"
            >
              View Description
            </button>
          </div>
        ...
        {showModal && (
          ...
          >
            {isModalLoading ? (
              ...
              <motion.div
                className="bg-white w-full h-full p-6 relative overflow-auto"
